...
@section('container')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Reset Data Surat Masuk</h5>
                ...
                <div class="modal-footer">
                    ...
                    <a class="btn btn-danger me-3" href="{{ route('reset-letter-in') }}">
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ url('admin/letter/surat-masuk/{id}/delete') }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <div class="modal-header">
                    ...
                    <div class="card card-header-actions mb-4">
                        <div class="card-header">
                                ...
                                <div class="col-6 gap-2 d-flex justify-content-end">
                                    <a class="btn btn-sm btn-blue" href="{{ route('create-masuk') }}">
                                        <i data-feather="plus-square"></i> &nbsp;
                                        Tambah Surat
                                    </a>
                                    <a class="btn btn-sm btn-primary" href="{{ route('print-surat-masuk') }}"
                                        target="_blank">
                                        <i data-feather="printer"></i> &nbsp;
                                        Cetak Laporan
                                    </a>
                                    @if (Auth::user()->role == 'Super Admin')
                                        <a class="btn btn-sm btn-pink resetIn">
                                            <i data-feather="trash-2"></i> &nbsp;
                                            Reset All Data
                                        </a>
                                    @endif
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a class="btn btn-primary btn-xs"
                                                    href="{{ route('letterin.edit', $items->id) }}">
                                                    <i class="fas fa-edit"></i> &nbsp; Ubah
                                                </a>
                                                <a class="btn btn-success btn-xs"
                                                    href="{{ route('detail-surat-masuk', $items->id) }}">
                                                    <i class="fa fa-search-plus"></i> &nbsp; Detail
                                                </a>
                                                <a class="btn btn-warning btn-xs"
                                                    href="{{ route('print-disposisi', $items->id) }}" target="_blank">
                                                    <i class="fa fa-print"></i> &nbsp; Cetak Disposisi
                                                </a>
                                                <button class="btn btn-danger btn-xs delete" value="{{ $items->id }}">
                                                    <i class="far fa-trash-alt"></i> &nbsp; Hapus
                                                </button>
                                                {{-- <form action="{{ route('letterin.destroy', $items->id) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                </form> --}}
...
@push('alert-script')
    <script>
        $(document).ready(function() {
            $(document).on('click', '.resetIn', function(e) {
                e.preventDefault();
